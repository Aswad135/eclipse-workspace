...
public class Main {
	...
	static Bank[] Banks = new Bank[10];
	static User[] Users = new User[3];
	static Entry[] Entries = new Entry[10];
	static int b = 0; // banks
	...
	public static Entry createEntry() throws ParseException {
		...
		System.out.print("Enter 1 for User entry or 2 for Bank entry: ");

		// Captured user input as integer
		choice = s.nextInt();

		// Validate user choice
		if (choice == 1) { // treat as user
			entryType = "user";
			System.out.println("User entry type selected.");
		} else if (choice == 2) { // treat as bank
			entryType = "bank";
		...
		// Proceed if user selected valid choice
		if (choice == 1 || choice == 2) {
			...
			Entries[e] = ENTRY; // push entry in to the array of entries
		...
		// return null if fail to create entry - should never get this far.
		return null;
	...
	public static void displayEntries() {
		...
		for (int i = 0; i < Entries.length; i++) {
			...
			System.out.println("Amount: " + Entries[i].getEntryAmount());
			System.out.println("Date: " + Entries[i].getEntryDate());
			System.out.println("Type: " + Entries[i].getEntryType());
		}
	}
	...
	public static Bank createNewBank() {
		...
		if (BANK.validateBank()) {
			Banks[b] = BANK; // add bank to banks array
	...
	public static void createUser() {
		...
		try {
			...
			for (int i = 0; i < Users.length; i++) {
				...
				if (Users[i].getId().equalsIgnoreCase(id) && Users[i] != null) {
	...
	public static Check createCheck(String userID) {
		...
		// create new check
		Check newCheck = new Check(userId, accountID, bankID, number, payee, amount, date);
	...
	public static void displayBanks() {
		...
		for (int i = 0; i < Banks.length; i++) {
			...
			System.out.println(Banks[i].getBankInfo());
		}
	}
}